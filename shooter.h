...
private:
    ...
    double av;// The angular-velocity of the shooter
    Color color;    // The color of the shooter

public:
    Shooter(double position_x, double position_y, double vx_=100, double av_=7.2, double height=30, double width=18, double head_radius=8, Color color_=COLOR(0, 255, 0))
    {
        // Shooter constructor
        color = color_;
    ...
    {
        // shoot a bullet
        return Bullet(head.getX(), head.getY(), body.getOrientation());
    }
    ...
    void rota(double t, bool is_rotating_right)
    {
        double ang_vel= av;
        ...
        //double new_body_x = body.getX() + ang_vel*t;
        //double new_head_x = head.getX() + ang_vel*t;
        ...
        body.rotate(ang_vel*t);
        double NHX=body.getX()+((body.getHeight()/2.0)*(sin(body.getOrientation())));
        double NHY=body.getY()-((body.getHeight()/2.0)*(cos(body.getOrientation())));
        head.moveTo(NHX, NHY);
    }
